"""
Crie um programa que leia nome, ano de nascimento e carteira
 de trabalho e cadastre-os (com idade) em um dicionário se por
 acaso a CTPS for diferente de 0, o dicionário receber´s também
 o ano de contratação e o salério. Calcule e acrescente, alé da
 idade, com quantos anos a pessoa vai se aposentar.
"""
from datetime import date
atual = date.today().year
dic = {}
dic['nome'] = input('Nome: ')
nasc = int(input('Ano de nascimento: '))
dic['idade'] = atual - nasc
dic['CTPS'] = int(input('Carteira de trabalho (0 não tem): '))
if dic['CTPS'] != 0:
    dic['contratação'] = int(input('Ano de contratação: '))
    dic['salário'] = float(input('Salário: '))
    dic['aposentadoria'] = (dic['contratação'] + 35) - atual + dic['idade']
print('='*30)
for k, v in dic.items():
    print(f'- {k} tem o valor {v}.')
